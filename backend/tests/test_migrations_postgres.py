...
def _venv_alembic() -> str:
    windows = Path(".venv/Scripts/alembic.exe")
    if windows.exists():
        return str(windows)
...
def test_alembic_upgrade_head_against_real_postgres() -> None:
    ...
    subprocess.run(
        [_venv_alembic(), "-c", "backend/alembic.ini", "upgrade", "head"],
        check=True,
        env=env,
        cwd=os.getcwd(),
    )
    url = env["AP_DATABASE_URL"].replace("+asyncpg", "").replace("+psycopg", "")
