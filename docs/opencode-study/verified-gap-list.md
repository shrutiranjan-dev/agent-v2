...
Verified overall parity: 75% (Tool system category raised to 95% by the File Diff/Review/Undo Batch 2; verified overall moved from 72% to 75% on the strength of revert approval gate, 409 contract, batch revert, Git fallback, and end-to-end round-trip smoke)
Latest batch: `File Diff/Review/Undo Batch 2` (DONE; see `implementation-roadmap.md` and the new `file_change_batch_2026_06_07_batch2` block in `flow-parity-matrix-verified.json` for the scope and validations). Closes the revert approval gate, 409 contract, batch revert, Git/VCS fallback, and end-to-end round-trip smoke.
...
Latest CI verifier hardening: `GitHub Actions Verifier Hardening` (DONE; see [`docs/ci.md`](../ci.md) "Repo-Local CI Verification" for the new state machine, the WORKFLOW diagnostic table, the SHA-pinning at selection time, the exponential backoff on transient REST errors, the rate-limit detection, the `-Once` and `-SelfTest` flags, the seven mock JSON fixtures in `scripts/testdata/ci-verifier/`, the new `mark-ci-validated.ps1` Process-based guarded invocation, and the updated Bash mirror in `scripts/check-github-actions.sh`). The previous "PowerShell polling bug" caveat is **RESOLVED**; the verifier now self-tests against seven fixtures (`empty`, `in_progress`, `missing_workflow`, `success`, `failure`, `duplicate_runs`, `wrong_sha`) and proves the current HEAD with `CI_STATUS_VERIFIED=true` via `mark-ci-validated.ps1`.
...
3. Real LSP CI follow-through is back to `REAL_LSP_CI_VALIDATED`. The mandatory `real-python-lsp-smoke` job exists in default CI, but the repo-local verifier checked commits `2564c64` and `ed13d13` through the public GitHub REST API and found `Repo Hygiene=success` while `CI=failure`, so `REAL_LSP_CI_VALIDATED` is not currently supportable. **STATUS UPDATE (file-change batch 170506f):** GitHub REST API confirmed `CI=success` (run 27079526756) and `Repo Hygiene=success` (run 27079526757) on commit `170506f`. The repo-local `check-github-actions.ps1` PowerShell script timed out on its appear-polling loop even though the runs were already successful, so this audit flips the markers to `CI_CI_VALIDATED` and `REPO_HYGIENE_CI_VALIDATED` on the strength of the public API. **STATUS UPDATE (verifier hardening):** the polling race has been fixed (see "Latest CI verifier hardening" above). The new verifier SHA-pins every selected run to the exact requested SHA, retries on transient REST errors with exponential backoff, detects GitHub rate-limit responses, and ships an in-script self-test against seven mock JSON fixtures. The new `mark-ci-validated.ps1` invokes the verifier via `[System.Diagnostics.Process]` and refuses to flip any doc marker unless the verifier exits 0; manually flipping the markers in the docs is a violation of the verification contract. The current `mark-ci-validated.ps1 -Once` run against HEAD prints `CI_STATUS_VERIFIED=true` and the `WORKFLOW` table shows the exact run ids that were used.
...
6. Local CI/release validation ergonomics: one-command `validate-local.ps1`, dedicated CLI/TUI CI job, failure artifacts on every CI job, repo hygiene now covers PowerShell parse, JSON parse, and line-ending policy.
7. **NEW** GitHub Actions verifier hardening: deterministic state machine, SHA-pinned run selection, exponential backoff on transient REST errors, rate-limit detection, seven-fixture self-test (`-SelfTest` returns `SELFTEST_RESULT=passed`), and a `mark-ci-validated.ps1` guard that refuses to flip any doc marker unless the verifier exits 0.

## Weakest Verified Areas

1. GitHub bot/workflow automation.
2. ~~CI green status (improved tooling; live green confirmation still pending the next push).~~ **RESOLVED** — the hardened verifier and `mark-ci-validated.ps1` guard now print `CI_STATUS_VERIFIED=true` against the current HEAD with no manual flipping.
3. Real LSP enabled-mode operation is validated locally for the Python `pylsp` path; `REAL_LSP_CI_VALIDATED` is supportable for the commits where the hardened verifier has independently proven green. TypeScript/JS LSP is still future.
