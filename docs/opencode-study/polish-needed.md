...
## File diff / review / undo (Batch 2)

- Current improvement: Batch 1 captured and reverted individual file changes durably. Batch 2 (DONE) closes the five concrete gaps that were on the Batch 1 polish list: revert returns HTTP 409 (not 500) for hash mismatch, the approval gate mirrors the `PermissionRequest` lifecycle (`AP_FILE_CHANGE_REVERT_REQUIRES_APPROVAL` default `true`, 202 `waiting_permission` response with `permission_request_id` + `approval_nonce`), a new `POST /file-changes/revert-batch` endpoint validates-all-before-apply with structured `{reverted, skipped, failed, restored_from_snapshots}` results, a `restore_from_git_head()` helper is the secondary fallback inside a Git working tree (gated by `AP_FILE_CHANGE_GIT_FALLBACK_ENABLED`), and `scripts/file-change-smoke.ps1`/`.sh` now perform a real end-to-end write+revert round-trip and print `FILE_CHANGES=round_trip_validated` (or `skipped_test_endpoint_disabled` when test endpoints are off). The CLI gets `agentv2 changes revert-batch` and `--approve`/`--permission-request-id` flows; the WebUI File Changes panel renders the `waiting_permission` state with an "Approve and retry" button and distinguishes conflict (409) / forbidden (403) / generic (500+) errors.
- Why it matters: every approved mutation tool now leaves a durable, auditable, revertible, permission-gated trail with secret redaction and a real round-trip smoke; users can review and undo risky changes (single or batched) from the UI, CLI, or API, with an explicit approval step that mirrors the existing permission policy.
- Exact files: `backend/app/file_changes/file_change_service.py`, `backend/app/api/routes_file_changes.py`, `backend/app/core/config.py`, `backend/app/core/events.py`, `backend/app/cli/api_client.py`, `backend/app/cli/main.py`, `backend/tests/fakes.py`, `backend/tests/test_file_changes_batch2.py`, `frontend/src/api/client.ts`, `frontend/src/components/FileChangesPanel.tsx`, `scripts/file-change-smoke.{ps1,sh}`, `scripts/validate-local.ps1`, `docs/opencode-study/{flow-parity-matrix-verified,current-parity-scorecard}.json`, `docs/opencode-study/{103-current-progress-audit,verified-gap-list,next-implementation-priorities,next-perfect-core-roadmap,remaining-work-percentage,implementation-roadmap}.md`.
- Exact recommended next fix:
  1. Frontend File Changes panel: add batch select checkbox per row + a "Revert selected" action that calls `revertFileChangesBatch` and surfaces per-change reasons.
  2. Add a memory/compaction live Qdrant smoke to raise Memory category from 70 to 75 (the lowest single ROI on the 80→90% path).
  3. Consider extending `revert_file_change` to also accept a `comment` (audit log) and persist it on the `FileChange` row.
- Risk if ignored: deployment drift in the new permission-gated revert flow could still hide behind the local smoke if CI does not exercise the approval + retry path end-to-end. Add a CI step that runs the `revert_file_change` round-trip with `AP_FILE_CHANGE_REVERT_REQUIRES_APPROVAL=true` and the approval retry flow.
