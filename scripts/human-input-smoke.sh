...
curl_json GET /health >/tmp/human-input-smoke-health.json || fail_or_skip "backend health is unreachable"

log "checking postgres and redis dependencies"
curl_json GET /health/dependencies >/tmp/human-input-smoke-dependencies.json
.venv/bin/python - <<'PY'
import json
from pathlib import Path

payload = json.loads(Path("/tmp/human-input-smoke-dependencies.json").read_text())
...
log "selecting model"
curl_json GET /models >/tmp/human-input-smoke-models.json
MODEL_NAME="$(.venv/bin/python - <<'PY'
import json
from pathlib import Path

models = json.loads(Path("/tmp/human-input-smoke-models.json").read_text()).get("models", [])
...
curl_json POST /sessions "{\"title\":\"Human input smoke\",\"agent_id\":\"general\",\"model_name\":\"${MODEL_NAME}\"}" >/tmp/human-input-smoke-session.json
SESSION_ID="$(.venv/bin/python - <<'PY'
import json
from pathlib import Path

print(json.loads(Path("/tmp/human-input-smoke-session.json").read_text())["session"]["id"])
PY
)"
...
if ! curl_json POST /human-input/test/seed "${SEED_BODY}" >/tmp/human-input-smoke-seed.json; then
...
print(json.loads(Path("/tmp/human-input-smoke-seed.json").read_text())["request"]["id"])
PY
)"

log "answering human input request ${REQUEST_ID}"
curl_json POST "/human-input/${REQUEST_ID}/answer" '{"answer":"yes"}' >/tmp/human-input-smoke-answer.json
.venv/bin/python - <<'PY'
import json
from pathlib import Path

payload = json.loads(Path("/tmp/human-input-smoke-answer.json").read_text())
...
for _ in $(seq 1 40); do
  curl_json GET "/sessions/${SESSION_ID}" >/tmp/human-input-smoke-detail.json
  if .venv/bin/python - <<'PY'
import json
from pathlib import Path

payload = json.loads(Path("/tmp/human-input-smoke-detail.json").read_text())
session = payload["session"]
tool_calls = payload.get("tool_calls", [])
events = []
try:
    events = json.loads(Path("/tmp/human-input-smoke-events.json").read_text()).get("events", [])
...
PY
  ...
  curl_json GET "/sessions/${SESSION_ID}/events" >/tmp/human-input-smoke-events.json || true
  sleep 1
done

curl_json GET "/sessions/${SESSION_ID}/events" >/tmp/human-input-smoke-events.json
.venv/bin/python - <<'PY'
import json
from pathlib import Path

detail = json.loads(Path("/tmp/human-input-smoke-detail.json").read_text())
events = json.loads(Path("/tmp/human-input-smoke-events.json").read_text()).get("events", [])
