...
curl_json GET /health >/tmp/memory-compaction-health.json || fail_or_skip "backend health is unreachable"

log "checking postgres and redis dependencies"
curl_json GET /health/dependencies >/tmp/memory-compaction-dependencies.json
.venv/bin/python - <<'PY'
import json
from pathlib import Path

payload = json.loads(Path("/tmp/memory-compaction-dependencies.json").read_text())
...
curl_json POST /sessions '{"title":"Memory compaction smoke","agent_id":"general"}' >/tmp/memory-compaction-session.json
SESSION_ID="$(.venv/bin/python - <<'PY'
import json
from pathlib import Path

print(json.loads(Path("/tmp/memory-compaction-session.json").read_text())["session"]["id"])
PY
)"
...
if ! curl_json POST /memory/test/compaction-seed "${SEED_BODY}" >/tmp/memory-compaction-seed.json; then
...
for _ in $(seq 1 40); do
  curl_json GET "/sessions/${SESSION_ID}/summaries" >/tmp/memory-compaction-summaries.json || true
  if .venv/bin/python - <<'PY'
import json
from pathlib import Path

payload = json.loads(Path("/tmp/memory-compaction-summaries.json").read_text())
...
curl_json GET "/sessions/${SESSION_ID}/summaries" >/tmp/memory-compaction-summaries.json
curl_json GET "/sessions/${SESSION_ID}/memory" >/tmp/memory-compaction-memory.json
curl_json GET "/sessions/${SESSION_ID}/events" >/tmp/memory-compaction-events.json
.venv/bin/python - <<'PY'
import json
from pathlib import Path

summaries = json.loads(Path("/tmp/memory-compaction-summaries.json").read_text()).get("summaries", [])
memory = json.loads(Path("/tmp/memory-compaction-memory.json").read_text()).get("memory_items", [])
events = json.loads(Path("/tmp/memory-compaction-events.json").read_text()).get("events", [])
